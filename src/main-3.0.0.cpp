...
public:
    ...
    std::vector<float> vertices;                 // OpenGL-friendly vertex data
    std::vector<unsigned int> line_indices;      // Edge indices (for GL_LINES)
...
const char* vertexShaderSource = R"glsl(
    #version 330 core
    layout(location = 0) in vec3 aPos;        // 从 vec2 改为 vec3
    uniform mat4 uMVP;

    void main() {
        gl_Position = uMVP * vec4(aPos, 1.0); // 不再补 z=0，直接使用 3D 坐标
    }
)glsl";
